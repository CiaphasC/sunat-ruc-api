...
public sealed class SunatClient
{
    ...
    private SunatClient(HttpClient h,IMemoryCache m,IDatabase? r){
        ...
        Encoding.RegisterProvider(System.Text.Encoding.CodePages.CodePagesEncodingProvider.Instance);
    }
    ...
    private async Task<RucInfo> SendAsync((string accion, params (string k,string v)[] ex) data){
        var form=new Dictionary<string,string>{{"accion",data.accion}};
        foreach(var (k,v) in data.ex)form[k]=v;
        string key=JsonSerializer.Serialize(form);
        if(_mem.TryGetValue(key,out RucInfo c))return c;
        if(_redis!=null&&_redis.StringGet(key)is var j and {HasValue:true})return JsonSerializer.Deserialize<RucInfo>(j)!;
